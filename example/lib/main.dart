...
class _MyAppState extends State<MyApp> with SingleTickerProviderStateMixin {
  ...
  Future<void> writeDemoFile(String mpqPath, String filePath) async {
    ...
    String? fileHandle = await SFileCreateFile(
        mpqHandle!, filePath.split(Platform.pathSeparator).last, data.lengthSync(), MPQ_FILE_COMPRESS);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      floatingActionButton: _selectedIndex == 2 ? null : FloatingActionButton(
        onPressed: () async {
          if (_selectedIndex == 0) {
            ...
            if(outputMPQ == null) {
              ...
              mpqHandle = await SFileCreateArchive(
                outputMPQ!,
        ...
        child: Icon([Icons.file_open_outlined, outputMPQ == null ? Icons.create_new_folder : Icons.save][_selectedIndex], key: ValueKey("icon$_selectedIndex"))),
      ),
      appBar: AppBar(
        title: const Text('Demo MPQ Utility', style: TextStyle(color: Colors.black54)),
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  const Text("About:"),
                  const Text("This is a demo app for the flutter_storm package, which is a Dart wrapper for the StormLib library", style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 10),
                  const Text("Flutter Storm Version:"),
                  Text("v${packageInfo.version}", style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 10),
                  const Text("StormLib Version:"),
                  Text(STORMLIB_VERSION, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 10),
                  const Text("StormLib License:"),
                  const Text("MIT License", style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 10),
                  ...
                  InkWell(
                    ...
                    child: Text(STORMLIB_REPO, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600))
                  ),
                  // Simple about page
                        ...
                        children: [
                          ...
                          TextSpan(
                            ...
                            recognizer: TapGestureRecognizer()
                              ..onTap = () { _launchUrl('https://discord.gg/shipofharkinian');
                            },
                          ),
                          const TextSpan(
                            text: " Team",
